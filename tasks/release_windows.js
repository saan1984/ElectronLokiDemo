...
var copyRuntime = function () {
    return projectDir.copyAsync('node_modules/electron-prebuilt/dist',
        readyAppDir.path(), { overwrite: true });
};

var packageBuiltApp = function () {
    var deferred = Q.defer();
    asar.createPackage(projectDir.path('build'),
        readyAppDir.path('resources/app.asar'), function() {
        deferred.resolve();
    });
...
var createInstaller = function () {
    ...
    //console.log("readyAppDir.path()",readyAppDir.path());
    //console.log("releasesDir.path(finalPackageName)",releasesDir.path(finalPackageName));
   ...
   // console.log("readyAppDir",readyAppDir);
